...
export default async function verificarErroConteúdoToken(request, response, next) {
    ...
    const usuário_token = await Usuário.findOne({ where: { email: request.email } });
    const usuário = await Usuário.findOne({ where: { cpf: cpf_encriptado } });
    if (usuário_token.email !== usuário.email) return response.status(401).json("Acesso não autorizado.");
    next();
}