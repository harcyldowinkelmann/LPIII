...
export default class ServiçosUsuário {
  ...
  static async verificarCadastroCompleto(usuário: Usuário) {
    switch (usuário.perfil) {
      case Perfil.MUSICO:
        const músico = await Músico.findOne({ where: { usuário: usuário.cpf }, relations: ["usuário"] });
        if (!músico) return false;
        return true;
      case Perfil.MAESTRO:
        const maestro = await Maestro.findOne({ where: { usuário: usuário.cpf }, relations: ["usuário"] });
  ...
  static async verificarRespostaCorreta(request, response) {
    try {
      ...
      const token = sign({ perfil: usuário.perfil, email: usuário.email },
        SENHA_JWT, { subject: usuário.nome, expiresIn: "1h" });
